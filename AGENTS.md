...
| 请求主链路 | `internal/gateway/handler.go:74` (`serve`) → `:142` (`dispatch`) | 鉴权→解码→路由→矩阵→凭据→Provider→回写 |
| 首字节后不许重试的实现点 | `internal/gateway/handler.go:196`、`relay.go` | `tracked.wrote` 是唯一判据 |
| 出站适配器接口 | `internal/provider/provider.go:34` | 同时拿 `Raw` 与 `Canonical`，适配器自己挑 |
| 启动装配 | `internal/gateway/build.go:32` | 未实现的协议族在此直接拒绝启动 |
...
| 符号 | 类型 | 位置 | 角色 |
|---|---|---|---|
| `degrade.Matrix.Check` | Method | `internal/degrade/matrix.go:390` | 全部处置裁决的唯一入口；fail-closed |
| `degrade.Matrix.BestOutbound` | Method | `internal/degrade/preference.go:236` | 候选上游排序 + 能力校验，返回 Verdict |
| `degrade.Route.Build` | Method | `internal/degrade/matrix.go:228` | 缺一格能力声明就报错 |
...
| `provider.Provider` | Interface | `internal/provider/provider.go:34` | `Kind()` + `Call()`，返回原始 `*httpx.Response` |
| `gateway.Handler.dispatch` | Method | `internal/gateway/handler.go:142` | Provider × 凭据两层 failover |
...
- **转正需三处同改**（ADR-0001）：代码 + `testdata/routes/<in>__<out>/` fixture +
  `matrix_test.go` 的 `TestImplementedRoutesAreExplicit` 名单。少一处就过不了 CI。
...
- 未实现的路径返回 **501**（PLANNED，还没建好），能力不支持返回 **422**。
...
- **当前状态**：14 条路径已登记，**只有 3 条已实现**（`openai.responses →
  openai.compat`、`openai.chat → openai.compat`、`dashscope.native →
  dashscope.native`，均为同源直通）。其余打过去是 501。
