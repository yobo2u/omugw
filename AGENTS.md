...
| 符号 | 类型 | 位置 | 角色 |
|---|---|---|---|
| `degrade.Matrix.Check` | Method | `internal/degrade/matrix.go:560` | 全部处置裁决的唯一入口；按入站坐标（协议 + 端点）裁决，fail-closed |
...
| `degrade.Matrix.Add` | Method | `internal/degrade/matrix.go:483` | 路径入库唯一入口：只收已 Build 的路径，拒绝 nil——Build 校验绕不过去 |
